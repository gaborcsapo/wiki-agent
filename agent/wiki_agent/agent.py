...
SYSTEM_PROMPT = (
    ...
    "- For facts tied to a specific country, language, or local topic, go to that "
    "language's Wikipedia edition FIRST by setting the `lang` code (e.g. a "
    "Hungarian place/person -> lang='hu', Icelandic -> 'is') — do not waste turns "
    "searching English first, as the fact is often only on the native edition. If "
    "the question is written in another language, search that edition. Once you "
    "find the right article, OPEN it with get_article and read its full text "
    "before answering — the specific fact is usually in the body, not the search "
    "snippet. Use English only as a fallback.\n"
