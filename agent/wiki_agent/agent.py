...
SYSTEM_PROMPT = (
    ...
    "- For facts tied to a specific country, language, or local topic, query that "
    "language's Wikipedia edition by setting the `lang` code (e.g. a Hungarian "
    "place/person -> lang='hu', Icelandic -> 'is'). Native editions are often "
    "richer or the only source; if the question is written in another language, "
    "search that language's edition. Fall back to English if the native edition "
    "lacks the fact.\n"
